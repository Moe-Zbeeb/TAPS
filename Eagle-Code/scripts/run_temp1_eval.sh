#!/usr/bin/env bash
# Evaluate all Eagle checkpoints at temperature=1 across 4 GPUs.
# GPU 0: MathInstruct   GPU 1: ShareGPT
# GPU 2: Mixed          GPU 3: Merged
...
CKPT=/home/zbibm/MOSS---Mixture-of-Speculative-Samplers/Eagle-Code/checkpoints
OUT=/home/zbibm/MOSS---Mixture-of-Speculative-Samplers/Eagle-Code/results/temp1
LOGDIR="$OUT/logs"
...
echo "=== Launching single-head evals in parallel ==="
run_single_head 0 MathInstruct Eagle-MathInstruct_20epochs          > "$LOGDIR/MathInstruct.log" 2>&1 &
run_single_head 1 ShareGPT     Eagle-ShareGPT_20epochs              > "$LOGDIR/ShareGPT.log"     2>&1 &
run_single_head 2 Mixed        Eagle-Sharegpt-Mathinstruct-20epochs > "$LOGDIR/Mixed.log"        2>&1 &
run_single_head 3 Merged       Eagle-Averaged-Checkpoint            > "$LOGDIR/Merged.log"       2>&1 &
...
for BENCH in "${BENCHMARKS[@]}"; do
    ...
    python -m eagle.evaluation.gen_dual_head_eval \
        --base-model-path "$BASE" \
        --ea-model-path1  "$CKPT/Eagle-MathInstruct_20epochs" \
        --ea-model-path2  "$CKPT/Eagle-ShareGPT_20epochs" \
...
python3 - "$OUT" "$OUT/tau_table_temp1.txt" <<'PYEOF'
import sys
import json
