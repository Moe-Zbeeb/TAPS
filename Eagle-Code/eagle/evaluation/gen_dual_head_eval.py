"""Evaluate dual-head EAGLE-2 with step-level confidence routing.

At each decode step both draft heads propose a tree; the one with higher mean
token confidence is verified.  Outputs a JSONL file compatible with
evaluate/compute_tau.py.
...
Usage:
    python -m eagle.evaluation.gen_dual_head_eval \
        ...
        --head2-name ShareGPT \
        --bench-name mt_bench \
        --answer-file results/mt_bench_dual.jsonl
"""
    ...
    for question in tqdm(questions):
        ...
        for i in range(num_choices):
            ...
            for j in range(len(question["turns"])):
                ...
                output_ids, new_token, idx = model.dual_eagenerate(
                    torch.as_tensor(input_ids).to(device),
                    temperature=temperature,
                    log=True,
                    is_llama3=True,
...
def main():
    ...
    parser.add_argument("--model-id", type=str, default="dual-eagle2")
