...
class Model(nn.Module):
    ...
    def topK_genrate(
        ...
        if return_entropy:
            probs = torch.softmax(last_headout.float(), dim=-1)
            ent = -torch.sum(probs * torch.log(probs + 1e-10), dim=-1)  # shape: [1]
            # Each of the top_k tokens from this distribution inherits the same entropy
            entropy_flat.append(ent[0].expand(top_k))
        ...
        for i in range(depth):
            ...
            if return_entropy:
                probs = torch.softmax(last_headout.float(), dim=-1)
                ent = -torch.sum(probs * torch.log(probs + 1e-10), dim=-1)  # shape: [top_k]
                # Each branch produces top_k children, each inheriting its parent's entropy
                entropy_flat.append(ent.repeat_interleave(top_k))
