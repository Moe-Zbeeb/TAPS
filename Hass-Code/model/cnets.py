...
class Model(nn.Module):
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, return_logprobs=False, return_confidence=False):
        ...
        if return_logprobs:
            logprob_steps = [last_p.expand(top_k, -1)[None]]
        ...
        for i in range(depth):
            ...
            ss_token.append(topk_index)
            if return_logprobs:
                logprob_steps.append(last_p[None])
        ...
        if return_logprobs:
            logprob_steps = torch.cat(logprob_steps, dim=0)
        ...
        if return_logprobs:
            depth = logprob_steps.shape[0]
            leaf = retrieve_indices.shape[0]
            gathered = []
            for li in range(leaf):
                path = retrieve_indices[li]
                lp_path = []
                for d in range(min(depth, path.shape[0])):
                    idx = path[d]
                    if idx < 0:
                        continue
                    if idx >= logprob_steps.shape[1]:
                        continue
                    lp_path.append(logprob_steps[d, idx])
                if lp_path:
                    gathered.append(torch.stack(lp_path, dim=0))
                else:
                    gathered.append(torch.empty(0, logprob_steps.shape[-1], device=logprob_steps.device))
            maxd = max(x.shape[0] for x in gathered) if gathered else 0
            if maxd == 0:
                gathered_logprobs = torch.empty(0, 0, logprob_steps.shape[-1], device=logprob_steps.device)
            else:
                padded = []
                for g in gathered:
                    pad_len = maxd - g.shape[0]
                    if pad_len > 0:
                        g = torch.cat([g, torch.zeros(pad_len, logprob_steps.shape[-1], device=g.device)], dim=0)
                    padded.append(g)
                gathered_logprobs = torch.stack(padded, dim=0)
            return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, gathered_logprobs
