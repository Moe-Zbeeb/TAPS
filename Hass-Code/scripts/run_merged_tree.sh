#!/usr/bin/env bash
# Evaluate merged-tree dual-head HASS.
...
MODEL_ID="${MODEL_ID:-merged_tree_hass}"
...
RESULTS_DIR="${RESULTS_DIR:-results/merged}"
...
echo "========================================================"
echo "Merged-tree dual-head HASS evaluation  [4 GPUs, parallel]"
...
for i in "${!BENCHES[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" python -m evaluation.gen_merged_tree_eval \
